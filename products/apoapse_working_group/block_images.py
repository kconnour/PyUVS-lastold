...
def make_histogram_equalized_grid_detector_image(data_block: int, standard=False) -> None:
    ...
    if standard:
        filename = f'{pu.orbit.make_orbit_block(data_block)}-heq-15bins.png'
    else:
        filename = f'{pu.orbit.make_orbit_block(data_block)}-heq.png'
...
def make_color_only_grid_detector_image(data_block: int, standard=False) -> None:
    ...
    if standard:
        filename = f'{pu.orbit.make_orbit_block(data_block)}-coloronly-15bins.png'
    else:
        filename = f'{pu.orbit.make_orbit_block(data_block)}-coloronly.png'
    plt.savefig(save_location / filename, dpi=300)
    plt.close(fig)
...
if __name__ == '__main__':
    ...
    for block in range(13500, 17000, 100):
